...
define ("SITE_NAME", "Pokemon-Castle"); // set Site name
define ("DB_HOST", "localhost"); // set database host
define ("DB_USER", "root"); // set database user
define ("DB_PASS",""); // set database password
define ("DB_NAME","db1225392-main"); // set database name
...
/*************** reCAPTCHA KEYS****************/
$publickey = "6LdjGcASAAAAAAYu0D4piB6NLdZtFwc3Z2D9PYcq";
$privatekey = "6LdjGcASAAAAAAMahkwz0aAL8MZ2xMzRAQkEbrDY";
...
********************************************************/
function exp_raten($intelligence, $level) {
	$erf_exp = floor($level * (9 - ($intelligence / $level)));
	//$erf_exp = floor((5 / ($intelligence / $level) * $level));
	return($erf_exp);
}
...
function page_protect() {
	...
	//Alle 20 Stunden kann der User sein Geld bekommen
	$abfrage = mysql_query("select online, money from users where id='$_SESSION[user_id]'");
	while($check_user = mysql_fetch_array($abfrage)){
		if ($check_user['online'] < date('Y-m-d H:i:s',time()-(20*60*60))){
			$check_user['money'] += 10;
			$online_new = date('Y-m-d H:i:s');
			mysql_query("update users set money='$check_user[money]', online='$online_new' where id='$_SESSION[user_id]'") or die(mysql_error());
		}
	}
}
